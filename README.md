...
<img
  alt="my skills"
  src="https://skillicons.dev/icons?theme=dark&perline=8&i=java,spring,python,django,fastapi,flask,ts,js,react,nextjs,vue,nodejs,html,css,postgresql,mysql,sqlite,aws,docker,linux,git,github,swift"
/>
...
```mermaid
flowchart LR
  A[Full Stack Development] --> B[Backend]
  A --> C[Frontend]
  A --> D[Database]
  A --> E[Cloud / DevOps]

  B --> B1[Java / Spring Boot]
  B --> B2[Python / Django / FastAPI]
  ...
  F[Teaching / Curriculum] --> F1[Java Training]
  F --> F2[Python Training]
  F --> F3[AWS Hands-on]
  F --> F4[Team Development]
```
...
```mermaid
flowchart TD
  A[Requirement Definition] --> B[Use Case]
  B --> C[Screen Design]
  C --> D[Database Design]
  D --> E[ER Diagram]
  E --> F[Routing Design]
...
```mermaid
flowchart TD
  Now[Now - Full Stack Development] --> S1[TypeScript / Next.js Frontend]
  S1 --> S2[Spring Boot 3 API Modernization]
  S2 --> S3[FastAPI / Docker Microservices]
  S3 --> S4[AWS CDK TypeScript IaC]
  S4 --> S5[AI Workflow / Business Automation]
  S5 --> S6[Reusable Teaching Platforms]
```
